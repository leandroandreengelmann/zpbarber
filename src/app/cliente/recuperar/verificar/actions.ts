...
export async function verifyClientOtpAction(
  ...
  redirect("/cliente/recuperar/nova-senha");
}

export async function resendClientOtpAction(formData: FormData) {
  const email = String(formData.get("email") ?? "").trim().toLowerCase();
  if (!email || !isEmail(email)) {
  ...
  redirect(
    `/cliente/recuperar/verificar?email=${encodeURIComponent(email)}&resent=1`
  );
}
